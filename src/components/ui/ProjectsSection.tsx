...
/**
 * The section pins itself (position: sticky) for PROJECTS.length viewport
 * heights of scroll. Real scroll position drives which slide is active, so
 * the browser handles all the input (wheel, trackpad, touch, keyboard,
 * screen readers) instead of a custom wheel/touch interceptor. Once the
 * user scrolls past the last slide's bucket the section naturally
 * unsticks and the rest of the page continues underneath it.
 */
export function ProjectsSection() {
  const trackRef = useRef<HTMLDivElement>(null);
  const stickyRef = useRef<HTMLElement>(null);
  const hasEntered = useInView(stickyRef, { once: true, margin: "-10% 0px" });
  ...
  // Map scroll position through the tall track to a slide index.
  useEffect(() => {
    let ticking = false;
  ...
  // Whenever the scroll-driven target moves ahead of what's on screen,
  // and nothing is currently mid-wipe, kick off the exit -> enter chain.
  // If the target keeps moving during the exit wipe, handleExitComplete
  // below reads the latest activeIndex, so fast scrolling settles on the
  // final slide instead of flashing through every slide in between.
  useEffect(() => {
  ...
  function scrollToIndex(idx: number) {
    const target = clamp(idx, 0, PROJECTS.length - 1);
    const track = trackRef.current;
    if (!track) return;
    const scrollableDistance = Math.max(1, track.offsetHeight - window.innerHeight);
    const trackTop = track.getBoundingClientRect().top + window.scrollY;
    const progress = (target + 0.5) / PROJECTS.length;
    window.scrollTo({ top: trackTop + progress * scrollableDistance, behavior: "smooth" });
  }
  ...
  return (
    <div ref={trackRef} style={{ height: `${PROJECTS.length * 100}vh` }} className="relative">
      <section
        ref={stickyRef}
        tabIndex={0}
        ...
        className="sticky top-0 h-screen w-full overflow-hidden bg-black font-mono text-white outline-none"
      >
